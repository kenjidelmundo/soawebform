...
@Component({
      ...
      <div class="list">
        <label class="row">
          <input
            ...
            (click)="$event.preventDefault(); select('NEW')"
          />
        ...
        <label class="row">
          <input
            ...
            (click)="$event.preventDefault(); select('RENEW')"
          />
        ...
        <label class="row">
          <input
            ...
            (click)="$event.preventDefault(); select('MOD')"
          />
  ...
  styles: [`
    ...
    .row { display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 700; font-size: 14px; line-height: 1.2; user-select: none; }
...
export class TxnTypeDialogComponent {
  ...
  // ✅ checkbox look but single-select, with toggle off
  select(v: TxnType) {
